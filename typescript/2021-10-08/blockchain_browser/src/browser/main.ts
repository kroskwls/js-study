...
const amoutEl = document.getElementById('amout') as HTMLInputElement;
...
(async function main(): Promise<void> {
	...
	function addTransaction() {
		blockchain.createTransaction({
			...
			amout: parseInt(amoutEl.value)
		});

		toggleState(false, false);
		pendingTransactionsEl.textContent = blockchain.pendingTransactions.map(t => `${t.sender} -> ${t.recipient}: $${t.amout}`).join('\n');
...
function toggleState(confirmation: boolean, transferForm: boolean): void {
	transferBtn.disabled = amoutEl.disabled = senderEl.disabled = recipientEl.disabled = transferForm;
	...
	return `
		<div class="block">
			...
			<div class="block__transactions">
				...
				<pre class="transactions-value">${block.transactions.map(t => `${t.sender} -> ${t.recipient}: $${t.amout}`)}</pre>
			</div>
		</div>
	`;
}