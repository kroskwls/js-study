...
export class PendingTransactionsPanel implements Renderable<Readonly<BlockchainNode>> {
	...
	render(node: Readonly<BlockchainNode>): TemplateResult {
		...
		const formattedTransactions = node.hasPendingTransactions ? formatTransactions(node.pendingTransactions) : `No pending transactions yet.`;

		return html`
			<h2>Pending transactions</h2>
			<pre class="pending-transactions__list">${formattedTransactions}</pre>
			<div class="pending-transaction__form">${UI.button('GENERATE BLOCK', shouldDisableGenerate)}
				</form>
			<div class="clear"></div>
		`;
	}
